...
const SECTIONS: Section[] = [
    ...
    questions: [
      {
        ...
        options: [
          {
            id: "i1a",
            label: "Fast-paced, innovative (startup/tech)",
            scores: { tech: 2, business: 1 },
          },
          {
            id: "i1b",
            label: "Structured, large organizations",
            scores: { business: 2, research: 1 },
          },
          {
            id: "i1c",
            label: "Studio or creative spaces",
            scores: { arts: 2 },
          },
          {
            id: "i1d",
            label: "Clinical or service settings",
            scores: { health: 2 },
          },
        ],
      },
      ...
      },
      {
        ...
        options: [
          {
            id: "i3a",
            label: "Often (frequent travel)",
            scores: { business: 1, health: 1 },
          },
          {
            id: "i3b",
            label: "Sometimes (occasional)",
            scores: { business: 1 },
          },
          {
            id: "i3c",
            label: "Rarely (mostly local)",
            scores: { research: 1, tech: 1 },
          },
        ],
      },
      {
    ...
    questions: [
      {
        ...
        type: "mcq",
        options: [
          { id: "p1a", label: "Alone", scores: { research: 1, tech: 1 } },
          {
            id: "p1b",
            label: "In teams",
            scores: { business: 1, health: 1, arts: 1 },
          },
        ],
      },
      {
        id: "p2",
        text: "How do you handle stressful deadlines?",
        type: "rating",
      },
      {
        id: "p3",
        ...
        type: "mcq",
        options: [
          { id: "p3a", label: "Introverted", scores: { research: 1 } },
          { id: "p3b", label: "Extroverted", scores: { business: 1, arts: 1 } },
        ],
      },
      { id: "p4", text: "Do you enjoy mentoring others?", type: "yesno" },
      {
        id: "p5",
        text: "Prefer predictable tasks or varied challenges?",
        type: "mcq",
        options: [
          { id: "p5a", label: "Predictable", scores: { business: 1 } },
          { id: "p5b", label: "Varied", scores: { tech: 1, arts: 1 } },
        ],
      },
    ],
  },

  {
    ...
    questions: [
      ...
      {
        id: "v3",
        text: "Do you prefer public sector or private sector?",
        type: "mcq",
        options: [
          { id: "v3a", label: "Public", scores: { research: 1, health: 1 } },
          { id: "v3b", label: "Private", scores: { business: 1, tech: 1 } },
        ],
      },
      {
        id: "v4",
        text: "Would you consider a career in non-profit or social work?",
        type: "yesno",
      },
      {
        id: "v5",
        text: "Do you want a role with high growth potential?",
        type: "yesno",
      },
    ],
  },

  {
    ...
    questions: [
      {
        id: "g1",
        text: "Do you plan to pursue higher studies (Masters/PhD)?",
        type: "yesno",
      },
      {
        id: "g2",
        text: "Are you interested in entrepreneurship?",
        type: "yesno",
      },
      {
        id: "g3",
        text: "How open are you to learning new technical skills?",
        type: "rating",
      },
      {
        id: "g4",
        text: "Do you see yourself in a leadership role in 10 years?",
        type: "yesno",
      },
    ],
  },
      ...
      for (const q of sec.questions) {
        ...
        if (q.type === "yesno") {
          if (val === true) {
            ...
            if (q.id.startsWith("i2") || q.id.startsWith("s3"))
              scores.arts += 2;
            if (q.id.startsWith("s1")) scores.tech += 2;
            if (q.id.startsWith("s4")) scores.health += 2;
            if (q.id.startsWith("s5") || q.id.startsWith("g4"))
              scores.business += 1;
        ...
        if (q.type === "rating") {
          ...
          if (q.id === "p2" || q.id === "v2") {
            ...
            if (r >= 4) scores.business += 1;
            else scores.arts += 0;
          }
          if (q.id === "g3") {
            if (r >= 4) scores.tech += 2;
            else if (r >= 3) scores.tech += 1;
  ...
  const results = useMemo(() => {
    ...
    const mapping: Record<Cluster, { label: string; careers: string[] }> = {
      tech: {
        ...
        careers: ["Software Engineer", "Data Scientist", "Systems Engineer"],
      },
      business: {
        label: "Business & Management",
        careers: ["Product Manager", "Business Analyst", "Marketing Manager"],
      },
      arts: {
        label: "Arts & Creativity",
        careers: ["UX Designer", "Content Creator", "Graphic Designer"],
      },
      health: {
        label: "Healthcare & Social Work",
        careers: ["Nurse", "Public Health Specialist", "Therapist"],
      },
      research: {
        label: "Research & Academia",
        careers: ["Research Scientist", "Academic", "Lab Technician"],
      },
    };
        ...
        <Card>
          <CardHeader>
            <div>
              ...
              <CardDescription>
                Take a short quiz to get personalized career and college
                recommendations. It takes ~10 minutes.
              </CardDescription>
            </div>
          </CardHeader>
          <CardContent>
            <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
              <div className="space-y-2">
                <p className="text-sm text-slate-700">
                  Your progress will be saved locally so you can continue later.
                </p>
                {hasSaved && (
                  <p className="text-sm text-slate-600">
                    We found a saved quiz. You can continue where you left off
                    or start fresh.
                  </p>
                )}
              </div>
        ...
        <Card>
          <CardHeader>
            <div className="flex items-center justify-between w-full">
              <div>
                <CardTitle>
                  Find your path — step {step + 1} of {SECTIONS.length}
                </CardTitle>
          ...
          <CardContent>
            <div className="relative min-h-[240px]">
              {SECTIONS.map((sec, idx) => (
                <div
                  key={sec.id}
                  className={`transition-all duration-300 ${getTransition(idx)} bg-background px-2 py-2`}
                >
                  <div className="space-y-4">
                    {sec.questions.map((q) => (
                      <div
                        key={q.id}
                        className="space-y-2 border-b last:border-b-0 pb-4"
                      >
                        <div className="flex items-center justify-between">
                        ...
                        {q.type === "yesno" && (
                          <div className="flex gap-3">
                            <button
                              ...
                              className={`rounded-md px-3 py-2 ${answers[q.id] === true ? "bg-sky-600 text-white" : "bg-white"} ${reviewMode ? "opacity-60 cursor-not-allowed" : ""}`}
                            >
                              Yes
                            </button>
                            <button
                              ...
                              className={`rounded-md px-3 py-2 ${answers[q.id] === false ? "bg-sky-600 text-white" : "bg-white"} ${reviewMode ? "opacity-60 cursor-not-allowed" : ""}`}
                            >
                              No
                            </button>
                          </div>
                          ...
                          <div className="flex items-center gap-2">
                            {[1, 2, 3, 4, 5].map((n) => (
                              <button
                                ...
                                className={`h-9 w-9 rounded-full ${answers[q.id] === n ? "bg-sky-600 text-white" : "bg-slate-100 text-slate-700"} ${reviewMode ? "opacity-60 cursor-not-allowed" : ""}`}
                              >
                                {n}
                              </button>
                            ))}
              ...
              <div className="flex items-center gap-3">
                ...
                {step === SECTIONS.length - 1 && !submitted && (
                  <Button
                    ...
                    className="bg-sky-600 text-white inline-flex items-center gap-2"
                  >
                    Submit
                  </Button>
                )}
                ...
                {submitted && results && (
                  <Button
                    onClick={() =>
                      window.alert(JSON.stringify(results, null, 2))
                    }
                    className="inline-flex items-center gap-2"
                  >
                    View Results
