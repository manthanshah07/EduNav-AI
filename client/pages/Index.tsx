...
export default function Index() {
  ...
  const urgencyFor = (days: number) => {
    if (days < 0)
      return { color: "bg-slate-200 text-slate-600", label: "Passed" };
    if (days < 7)
      return { color: "bg-red-50 text-red-700", label: `Due in ${days}d` };
    if (days <= 30)
      return { color: "bg-amber-50 text-amber-700", label: `Due in ${days}d` };
    return {
    ...
    () => ({
      ...
      message:
        completionPercent >= 80
          ? "You're almost at the next level!"
        ...
        <div className="relative px-6 py-6 sm:px-8 sm:py-8 lg:px-10 lg:py-10">
          <div className="flex flex-col items-start justify-between gap-4 md:flex-row md:items-center">
            <div>
              ...
              <h1 className="mt-1 text-2xl font-extrabold sm:text-3xl lg:text-3xl">
                Welcome back, Alex!
              </h1>
              <p className="mt-2 max-w-2xl text-white/95">
                Pick up where you left off. Continue your career quiz, explore
                college matches, and stay on top of upcoming deadlines.
              </p>
            </div>
          ...
          <CardHeader>
            <div className="flex items-center justify-between">
              <div>
                ...
                <CardDescription>
                  {submitted
                    ? "Career Roadmap Ready!"
                    : `You're ${completionPercent}% of the way to your personalized career roadmap.`}
                </CardDescription>
              </div>
            ...
            <div className="flex items-center gap-4">
              ...
              <div className="flex flex-col">
                <span className="text-sm font-medium text-slate-700">
                  {submitted ? 100 : completionPercent}%
                </span>
                <span className="text-xs text-slate-500">{level.message}</span>
              </div>
            </div>
            <div className="mt-4 flex flex-wrap gap-3">
              <Button size="sm" className="rounded-full" onClick={onContinue}>
                Continue Quiz
              </Button>
              <Button
                size="sm"
        ...
        <Card>
          ...
          <CardContent>
                ...
                return (
                  ...
                  >
                    <div className="flex items-center gap-2">
                      <CheckCircle2
                        className={`h-4 w-4 ${days < 7 ? "text-red-500" : "text-emerald-500"}`}
                      />
                      <span className="font-medium">{d.title}</span>
                    </div>
                    <div className="flex items-center gap-2">
                      <span
                        className={`rounded-full px-2 py-0.5 text-xs ${urgency.color}`}
                      >
                        {urgency.label}
                      </span>
            ...
            <div className="mt-4">
              <Button
                variant="ghost"
                className="group w-full justify-start px-0 text-sky-700 hover:bg-transparent hover:text-sky-800"
              >
                View timeline
